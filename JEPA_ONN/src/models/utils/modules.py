...
class Attention(nn.Module):
    def __init__(
        ...
        if qkv_backend == "electronic":
            self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
            self.optical_qkv = None
        elif qkv_backend == "fsonn_tdm":
            if optical_config is None:
                raise ValueError("optical_config is required for qkv_backend=fsonn_tdm")
            if optical_config.input_dim != dim:
                raise ValueError("optical input_dim must match attention dim")
            self.qkv = None
            self.optical_qkv = TimeDivisionFSONN(optical_config)
        else:
            raise ValueError(f"unsupported qkv_backend: {qkv_backend}")
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop_prob = proj_drop
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(
        ...
        B, N, C = x.shape
        if self.qkv_backend == "electronic":
            qkv_source = self.qkv(x)
        else:
            qkv_source = self.optical_qkv(x)
        captured = {}
        if capture_nodes is not None and "qkv" in capture_nodes:
            if (
                self.qkv_backend == "electronic"
                and not qkv_include_bias
            ):
                captured["qkv"] = F.linear(
                    x, self.qkv.weight, bias=None
                )
            else:
                captured["qkv"] = qkv_source
        qkv = qkv_source.reshape(
        ...
        if self.use_sdpa:
            ...
            x = attn @ v
        x = x.transpose(1, 2).reshape(B, N, C)
        if capture_nodes is not None and "attention_output" in capture_nodes:
            captured["attention_output"] = x
        x = self.proj(x)
        proj_output = x
        x = self.proj_drop(x)
        if capture_nodes is not None and "post_output" in capture_nodes:
            captured["post_output"] = x
        if capture_nodes is not None:
            return x, attn, captured
        if return_proj_output:
            return x, attn, proj_output
        return x, attn
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if capture_nodes is None and qkv_include_bias:
            y_result = self.attn(
                self.norm1(x),
                mask=mask,
                return_proj_output=return_proj_output,
            )
        else:
            y_result = self.attn(
                self.norm1(x),
        ...
        if capture_nodes is not None:
            y, attn, captured = y_result
        elif return_proj_output:
            y, attn, proj_output = y_result
        else:
            y, attn = y_result
        if return_attention:
            return attn
        ...
        if capture_nodes is not None:
            if "block_output" in capture_nodes:
                captured["block_output"] = x
            return x, captured
        if return_proj_output:
